...
function isExists(barcodeChildElement) {
    const result = getDataSource(barcodeChildElement);
    if (result == null)
        return false;
    else return true;
}
...
function createReceipt(barcodes) {
    ...
    for (let i = 0; i < barcodes.length; i++) {
        ...
        const good = getDataSource(barcodes[i].id);
...
module.exports = {printReceipt, createReceipt, countGoods, isExists, getDataSource};